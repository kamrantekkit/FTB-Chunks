...
public interface FTBChunksWorldConfig {
	...
	BooleanValue ENABLE_TEAM_OFFLINE_PROTECTION = CONFIG.addBoolean("enable_team_offline_protection", true).comment("If true, teams can protect their claims even when no players are online. If false, only the team owner can protect their claims while offline.");
	IntValue TEAM_PROTECTION_ARMING_TIME = CONFIG.addInt("team_protection_arming_time", 0, 0, Integer.MAX_VALUE)
			.comment("Time in mins to wait before a team protection is armed after team log out", "If set to 0, the protection is armed immediately.");
